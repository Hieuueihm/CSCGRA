...
OUT = ROOT / "CSCGRA_gp_opt" / "tests" / "run1" / "m128n256k8_golden_mu3.vh"
...
def run_reference_alg(tb_alg: int, phi: list[list[int]], y: list[int], n_size: int, k_size: int, scale_q: int) -> list[int]:
    ...
    for _ in range(iter_limit):
        if tb_alg == 0:  # OMP
            ...
            support = prune_by_abs(temp_x, k_size)
            x, residual = ls_fit_quantized(phi, y, support, n_size)
        elif tb_alg == 5:  # GP grad-step RTL: x += (Phi^T r >>> MU_SHIFT), prune K, update residual
            scores = corr_scores(phi, residual, n_size)
            x = [sat_s24(x[col_idx] + (s24(scores[col_idx]) >> MU_SHIFT)) for col_idx in range(n_size)]
            support = prune_by_abs(x, k_size)
            keep = set(support)
...
def main() -> None:
    ...
    # Dedicated M=128/N=256/K=8 functional-golden vector.
    # Use a fixed 8-sparse signal and generate y = Phi*x in the same 24-bit fixed-point flow.
    x_true = [0] * 256
    for idx, val in [(18, -0x1100), (46, 0x0C00), (74, -0x1600), (91, 0x1300), (107, 0x0F00), (145, -0x1400), (182, 0x1000), (227, 0x1800)]:
        x_true[idx] = val
    y_by_case = []
    for m_size, n_size, _k_size in CASES:
        phi_for_y = make_phi(m_size, n_size, seed, scale_q)
        y_case = []
        for row in phi_for_y:
            acc = sum(row[col_idx] * x_true[col_idx] for col_idx in range(n_size))
            y_case.append(sat_s24(acc >> Q))
        y_by_case.append(y_case)

    lines: list[str] = []
    lines.append("// Auto-generated by scripts/export_m128n256k8_golden_gpopt.py")
    ...
    lines.append("localparam integer KSWEEP_GOLD_CASES = 1;")
    ...
    for elem_idx, value in enumerate(y_by_case[0]):
        lines.append(f"  {elem_idx}: ksgold_y = {hex24(value)};")
