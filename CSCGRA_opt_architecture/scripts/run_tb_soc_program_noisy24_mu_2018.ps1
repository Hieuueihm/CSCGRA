param(
    ...
    [string]$OutDir = "D:\vivado_pj\CSCGRA_noisy_mu\runs\tb_soc_program_noisy24_mu_2018"
)

$ErrorActionPreference = "Stop"
$Root = "D:\vivado_pj\CSCGRA_noisy_mu"
$RtlDir = Join-Path $Root "CSCGRA.srcs\sources_1\new"
$Tb = Join-Path $Root "tests\tb_soc_program_noisy24_mu.v"
...
& (Join-Path $VivadoBin "xelab.bat") --timescale 1ns/1ps --override_timeunit --override_timeprecision tb_soc_program_noisy24_mu -s tb_soc_program_noisy24_mu *> "xelab.stdout.log"
if ($LASTEXITCODE -ne 0) { Get-Content "xelab.stdout.log" -Tail 120; exit $LASTEXITCODE }

& (Join-Path $VivadoBin "xsim.bat") tb_soc_program_noisy24_mu -runall *> "xsim.stdout.log"
