...
$workName = if ($EnableProperties) { "m11_gp_phase_property_xsim" } else { "m11_gp_phase_xsim" }
...
if ($LASTEXITCODE -ne 0) { throw "M11 GP program generation failed" }
Copy-Item -Force -LiteralPath `
    (Join-Path $repoRoot "reports\v3\context_images\program_05_phase.mem") `
    -Destination $workRoot

$sources = @(
    "rtl\v3\reconstruction_control\reconstruction_phase_controller.v",
    "verification\v3\m11\tb_m11_gp_phase_replay.sv"
...
try {
    foreach ($source in $sources) {
        ...
        if ($LASTEXITCODE -ne 0) { throw "M11 GP phase compile failed: $source" }
    }
    $snapshot = if ($EnableProperties) {
        "m11_gp_phase_property_snapshot"
    } else {
        "m11_gp_phase_snapshot"
    }
    & $xelab tb_m11_gp_phase_replay -s $snapshot -debug typical
    if ($LASTEXITCODE -ne 0) { throw "M11 GP phase elaboration failed" }
...
$logName = if ($EnableProperties) {
    "gp_phase_property_xsim.log"
} else {
    "gp_phase_xsim.log"
}
Set-Content -LiteralPath (Join-Path $reportRoot $logName) -Value $output
if ($code -ne 0 -or $joined -match "FAIL:|Assertion violation") {
    throw "M11 GP phase replay failed"
}
if ($joined -notmatch "M11 GP PHASE IMAGE REPLAY PASS") {
    throw "M11 GP phase PASS marker missing"
}
Write-Host "VIVADO M11 GP PHASE XSIM PASS"
