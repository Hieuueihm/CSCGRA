...
$workName = if ($EnableProperties) { "m11_mp_phase_property_xsim" } else { "m11_mp_phase_xsim" }
...
if ($LASTEXITCODE -ne 0) { throw "M11 MP program generation failed" }
Copy-Item -Force -LiteralPath `
    (Join-Path $repoRoot "reports\v3\context_images\program_07_phase.mem") `
    -Destination $workRoot

$sources = @(
    "rtl\v3\reconstruction_control\reconstruction_phase_controller.v",
    "verification\v3\m11\tb_m11_mp_phase_replay.sv"
...
try {
    foreach ($source in $sources) {
        ...
        if ($LASTEXITCODE -ne 0) { throw "M11 MP phase compile failed: $source" }
    }
    $snapshot = if ($EnableProperties) {
        "m11_mp_phase_property_snapshot"
    } else {
        "m11_mp_phase_snapshot"
    }
    & $xelab tb_m11_mp_phase_replay -s $snapshot -debug typical
    if ($LASTEXITCODE -ne 0) { throw "M11 MP phase elaboration failed" }
...
$logName = if ($EnableProperties) {
    "mp_phase_property_xsim.log"
} else {
    "mp_phase_xsim.log"
}
Set-Content -LiteralPath (Join-Path $reportRoot $logName) -Value $output
if ($code -ne 0 -or $joined -match "FAIL:|Assertion violation") {
    throw "M11 MP phase replay failed"
}
if ($joined -notmatch "M11 MP PHASE IMAGE REPLAY PASS") {
    throw "M11 MP phase PASS marker missing"
}
Write-Host "VIVADO M11 MP PHASE XSIM PASS"
