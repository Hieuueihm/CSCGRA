...
text=VH.read_text(); text=re.sub(r'localparam integer NOISY_GP_SEED = \d+;', f'localparam integer NOISY_GP_SEED = {TARGET};', text)
def repl(func, vals):
 global text
 start=text.index(f'function signed [23:0] {func};'); cstart=text.index('        case(idx)',start); default=f"        default: {func} = 24'sd0;"; dend=text.index(default,cstart); eline=text.index('\n',dend)
 body=['        case(idx)']+[f"        {i}: {func} = 24'sh{(int(v)&MASK24):06x};" for i,v in enumerate(vals)]+[default]
 text=text[:cstart]+'\n'.join(body)+text[eline:]
repl('NOISY_GP_Y',entry['y_noisy_q24']); repl('NOISY_GP_XHAT',entry['x_hat_q24'])
lines=[ln for ln in text.splitlines() if not ln.startswith('// GP golden override:')]
lines.insert(1,'// GP golden override: Python fixed-point grad-step reference seed434 (<500), RTL Phi, see analysis/reconstruction_quality_noisy24/noisy_lfsr_24bit_gp_grad_step_fixed_k16_seed434.json')
VH.write_text('\n'.join(lines)+'\n')
print('seed',entry['seed'],'snr',entry['snr_db'],'overlap',entry['overlap']); print('wrote',OUT)
