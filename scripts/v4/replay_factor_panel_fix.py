"""Replay immutable full-program PIO/DMA traces for the bounded output-slice fix."""
import hashlib
import json
...
CHANGED = ROOT / "rtl/v4/dataflow/factor_panel_service.sv"
...
def main():
    ...
    for name, expected in baseline["source_hashes_after"].items():
        ...
        if source.suffix in (".sv", ".v", ".vh", ".svh") and sha(source) != expected:
            drift.append(source)
    if drift != [CHANGED]:
        raise ValueError(f"Expected only factor-panel RTL change, found {drift}")
    directory = Path(tempfile.mkdtemp(prefix="factor_panel_fix_", dir=ROOT / "work"))
    ...
    evidence = dict(status="RUNNING", source_hashes_before=hashes, baseline=str(BASE),
                    ...
                    changed_source=dict(path=str(CHANGED), before=baseline["source_hashes_after"][str(CHANGED)], after=sha(CHANGED)),
                    runs=[], commands=[])
    def save():
