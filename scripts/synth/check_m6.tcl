if {$argc < 2} {
    error "usage: check_m6.tcl <repo_root> <report_dir> ?top ...?"
}
...
set constraint_file [file join $repo_root constraints v3 m6_ooc.xdc]
set rtl_sources [list \
    [file join $repo_root rtl v3 cgra pe_alu.v] \
    [file join $repo_root rtl v3 cgra phi_pe_alu.v] \
    [file join $repo_root rtl v3 cgra pe_local_register_file.v] \
    [file join $repo_root rtl v3 cgra registered_switchbox.v] \
    [file join $repo_root rtl v3 cgra pe_tile.v] \
    [file join $repo_root rtl v3 cgra phi_pe_tile.v] \
    [file join $repo_root rtl v3 cgra cgra_row.v] \
    [file join $repo_root rtl v3 cgra cgra_cluster.v] \
    [file join $repo_root rtl v3 cgra cgra_cluster_pair.v] \
    [file join $repo_root rtl v3 cgra cgra_result_buffer.v]]
set top_names [list pe_tile phi_pe_tile cgra_cluster cgra_cluster_pair cgra_result_buffer]
if {$argc >= 3} {
    set top_names {}
...
foreach top_name $top_names {
    ...
    close $fp
    puts "M6 $top_name synthesis PASS: WNS=$wns BRAM36=$ram36_count BRAM18=$ram18_count DSP=$dsp_count"
    close_project
}
