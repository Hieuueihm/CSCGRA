if {$argc != 2} { error "usage: check_m9b.tcl <repo_root> <report_dir>" }
...
set rtl_sources [list \
    ...
    [file join $repo_root rtl v3 selection m9b_support_subsystem.v]]
...
synth_design -top m9b_support_subsystem -part $part_name -mode out_of_context -flatten_hierarchy rebuilt
...
if {[llength $paths] == 0} { error "no setup path for m9b_support_subsystem" }
set wns [get_property SLACK [lindex $paths 0]]
if {$wns < 1.0} { error "m9b_support_subsystem WNS below +1.0 ns: $wns" }
...
if {$dsp != 0} { error "m9b_support_subsystem expected 0 DSP, got $dsp" }
if {$r36 > 1 || $r18 != 0} { error "m9b_support_subsystem BRAM gate failed: $r36/$r18" }
set fp [open [file join $report_dir summary.txt] w]
puts $fp "part=$part_name"
puts $fp "top=m9b_support_subsystem"
...
close $fp
puts "M9b synthesis PASS WNS=$wns LUT=$lut FF=$ff BRAM36=$r36 BRAM18=$r18 DSP=$dsp"
close_project
