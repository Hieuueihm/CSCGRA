...
$reportDir = Join-Path $repoRoot "reports\v3\axilite_control_vivado"
$workDir = Join-Path $repoRoot "work\synth\axilite_control"
New-Item -ItemType Directory -Force -Path $reportDir, $workDir | Out-Null
$tcl = Join-Path $PSScriptRoot "check_axilite_control.tcl"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "AXI-Lite reconstruction control Vivado check failed"
    }
} finally {
    Pop-Location
}

Write-Host "AXI-Lite reconstruction control Vivado check PASS"
