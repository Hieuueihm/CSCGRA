...
$reportDir = Join-Path $repoRoot "reports\v3\m7_vivado"
$workDir = Join-Path $repoRoot "work\synth\m7"
New-Item -ItemType Directory -Force -Path $reportDir, $workDir | Out-Null
$tcl = Join-Path $PSScriptRoot "check_m7.tcl"
...
try {
    ...
    & $vivado @arguments
    if ($LASTEXITCODE -ne 0) { throw "M7 Vivado synthesis check failed" }
} finally {
    Pop-Location
}
Write-Host "M7 Vivado synthesis check PASS"
