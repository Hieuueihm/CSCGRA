...
$reportDir = Join-Path $repoRoot "reports\v3\m4_vivado"
$workDir = Join-Path $repoRoot "work\synth\m4"
New-Item -ItemType Directory -Force -Path $reportDir, $workDir | Out-Null
$tcl = Join-Path $PSScriptRoot "check_m4.tcl"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "M4 Vivado synthesis check failed"
    }
} finally {
    Pop-Location
}

Write-Host "M4 Vivado synthesis check PASS"
