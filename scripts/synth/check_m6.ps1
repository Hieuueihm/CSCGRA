...
$reportDir = Join-Path $repoRoot "reports\v3\m6_vivado"
$workDir = Join-Path $repoRoot "work\synth\m6"
New-Item -ItemType Directory -Force -Path $reportDir, $workDir | Out-Null
$tcl = Join-Path $PSScriptRoot "check_m6.tcl"
...
try {
    ...
    & $vivado @arguments
    if ($LASTEXITCODE -ne 0) { throw "M6 Vivado synthesis check failed" }
} finally {
    Pop-Location
}
Write-Host "M6 Vivado synthesis check PASS"
