...
module factor_store (
    ...
    generate for(bank=0;bank<16;bank=bank+1) begin : banks
        ...
        for(port=0;port<2;port=port+1) begin : ports
            ...
            wire [9:0] write_address={1'(port),fill_address};
            wire [9:0] request_address={1'(port),req_addr[LOGICAL*9 +: 9]};
            reg [26:0] data;
            assign ram_data[LOGICAL*27 +: 27]=data;
            always @(posedge clk) begin
                if(fill_write&&fill_mask[fill_lane]) ram[write_address]<=fill_data[int'(fill_lane)*27 +: 27];
                else if(take&&req_write&&request_fault==0&&req_mask[LOGICAL])
                    ram[request_address]<=req_data[LOGICAL*27 +: 27];
                if(take)
