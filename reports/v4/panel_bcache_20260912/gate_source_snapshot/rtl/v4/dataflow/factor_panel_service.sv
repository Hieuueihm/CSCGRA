...
reg [863:0] factor_response_data;
...
integer request_lane,bank_offset;
...
wire [863:0] dot_result_data;
genvar dot_lane;
generate for(dot_lane=0;dot_lane<32;dot_lane=dot_lane+1)begin: dot_result_lanes
    if(dot_lane<8)begin: r4_lane
        assign dot_result_data[dot_lane*27+:27]=narrow_dot ? fabric_out_data[(dot_lane*4)*27+:27] : fabric_out_data[dot_lane*27+:27];
    end
    else begin: r1_lane
        assign dot_result_data[dot_lane*27+:27]=narrow_dot ? 27'b0 : fabric_out_data[dot_lane*27+:27];
    end
end endgenerate
...
wire signed [11:0] cache_read_base=$signed({1'b0,col_position})-$signed({1'b0,col_start});
wire [863:0] cache_read_rotation[0:5];
wire [863:0] cache_read_window=cache_read_rotation[5];
assign vector_rotation[0]=vec_rsp_data;
assign cache_rotation[0]=dot_phase ? dot_result_data : fabric_out_data;
genvar cache_bank,cache_stage;
generate
    for(cache_bank=0;cache_bank<32;cache_bank=cache_bank+1)begin: cache_read_banks
        wire signed [7:0] read_block=$signed(cache_read_base[11:5])+
            (cache_bank<cache_read_base[4:0] ? 8'sd1 : 8'sd0);
        assign cache_read_rotation[0][cache_bank*27+:27]=
            read_block==8'sd0 ? b_cache[cache_bank] :
            read_block==8'sd1 ? b_cache[32+cache_bank] :
            read_block==8'sd2 ? b_cache[64+cache_bank] : 27'bx;
        for(cache_stage=0;cache_stage<5;cache_stage=cache_stage+1)begin: rotation_stages
            assign cache_read_rotation[cache_stage+1][cache_bank*27+:27]=cache_read_base[cache_stage] ?
                cache_read_rotation[cache_stage][((cache_bank+(1<<cache_stage))%32)*27+:27] :
                cache_read_rotation[cache_stage][cache_bank*27+:27];
        end
    end
endgenerate
...
always @* begin
    ...
    for(comb_lane=0;comb_lane<32;comb_lane=comb_lane+1)begin
        ...
        if(matvec&&comb_lane<int'(dot_width))mat_output_nonzero=mat_output_nonzero||(dot_result_data[comb_lane*27+:27]!=0);
        if(feed_slot_valid&&range_operation&&comb_lane<32)begin
            ...
            if(narrow_rank)begin
                ...
                if(comb_lane<int'(rank_width)*2&&comb_relative<int'(row_count))begin
                    ...
                    fabric_in_b[comb_lane*27+:27]=cache_read_window[(comb_lane>>1)*27+:27];
                ...
                fabric_in_b[comb_lane*27+:27]=cache_read_window[comb_lane*27+:27];
                fabric_in_a[(16+comb_lane)*27+:27]=slot_factor[feed_slot][comb_lane*27+:27];
                fabric_in_mask[comb_lane]=1;
                fabric_in_mask[16+comb_lane]=1;
            end
        ...
        if(scale_feed_valid&&comb_lane<int'(panel_width))begin
            fabric_in_a[comb_lane*27+:27]=cache_read_window[comb_lane*27+:27];
            fabric_in_b[comb_lane*27+:27]=scalar_a;
            fabric_in_mask[comb_lane]=1;
        end
    end
    for(comb_bank=0;comb_bank<32;comb_bank=comb_bank+1)if(request_valid)begin
        if(range_operation)begin
            request_lane=(comb_bank-int'(row_start)-int'(col_start))&31;
            comb_relative=int'(request_row)*32+request_lane;
            comb_row=range_row ? int'(col_start) : int'(row_start)+comb_relative;
            comb_col=range_row ? int'(row_start)+comb_relative : int'(col_start);
            if(comb_relative<int'(row_count))begin
            ...
            bank_offset=(comb_bank-int'(row_start)-int'(request_row[2:0])-int'(col_position))&31;
            request_lane=(bank_offset&7)*4+(bank_offset>>3);
            comb_relative=(int'(request_row>>3)*32)+int'(request_row[2:0])+8*(request_lane&3);
            comb_row=int'(row_start)+comb_relative;
            comb_col=int'(col_position)+(request_lane>>2);
            if(request_lane<int'(dot_width)*4&&comb_relative<int'(row_count))begin
                panel_req_mask[comb_bank]=1;
                panel_req_addr[comb_bank*9+:9]=9'(3*comb_row+(comb_col>>5));
            end
        end
        else if(narrow_rank)begin
            bank_offset=(comb_bank-int'(row_start)-(int'(request_row>>3)*16)-int'(request_row[2:0])-int'(col_position))&31;
            request_lane=(bank_offset&7)*2+(bank_offset>>3);
            comb_relative=(int'(request_row>>3)*16)+int'(request_row[2:0])+8*(request_lane&1);
            comb_row=int'(row_start)+comb_relative;
            comb_col=int'(col_position)+(request_lane>>1);
            ...
            request_lane=(comb_bank-int'(row_start)-int'(request_row)-int'(col_position))&31;
            comb_row=int'(row_start)+int'(request_row);
            comb_col=int'(col_position)+request_lane;
            ...
            A_WAIT:if(vec_rsp_valid)begin
                if(vec_rsp_fault!=0)fail(`CSR_KERNEL_FAULT_OPERAND,vec_rsp_fault);
                    ...
                    if(range_template)begin
                        for(seq_lane=0;seq_lane<128;seq_lane=seq_lane+1)
                        ...
                        for(lane=0;lane<128;lane=lane+1)
                            if((lane>>5)==int'(source_bundle)&&source_mask[lane&31])
                                a_cache[lane]<=vec_rsp_data[(lane&31)*27+:27];
            ...
            B_WAIT:if(vec_rsp_valid)begin
                if(vec_rsp_fault!=0)fail(`CSR_KERNEL_FAULT_OPERAND,vec_rsp_fault);
                    ...
                    for(lane=0;lane<96;lane=lane+1)
                        if((lane>>5)==int'(source_bundle)&&source_mask[lane&31])
                            b_cache[lane]<=vec_rsp_data[(lane&31)*27+:27];
                ...
                fail((fabric_fault_seen||terminal_error!=0||fabric_done_fault!=0)?`CSR_KERNEL_FAULT_FABRIC:`CSR_KERNEL_FAULT_IDENTITY,terminal_error!=0?terminal_error:fabric_done_fault);
                    ...
                    if(panel_rsp_valid&&panel_rsp_ready) begin
                        if(!response_is_write) begin
                            ...
                            slot_factor[response_slot]<=factor_response_data;
                    ...
                    if(fabric_done_valid&&fabric_done_ready) begin
                        if(range_operation) begin
                            ...
                            if(project) begin
                                ...
                                if(narrow_dot)begin
                                    candidate_data<=0;
                                    for(lane=0;lane<32;lane=lane+1)if(lane<int'(dot_width))candidate_data[lane*27+:27]<=dot_result_data[lane*27+:27];
                                end
