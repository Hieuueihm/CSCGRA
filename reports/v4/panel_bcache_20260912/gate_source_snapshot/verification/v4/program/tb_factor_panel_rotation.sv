...
module tb_factor_panel_rotation;
    ...
    reg range_mode=0,dot_mode=0,rank_mode=0,dot_phase_value=0;
    reg [863:0] expected_response,expected_write,expected_vector,expected_cache;
    ...
    factor_panel_service dut(.clk(1'b0),.rst(1'b1),.cancel(1'b0),
        .req_valid(1'b0),.panel_rsp_data(response_data),.vec_rsp_data(response_data),.fabric_out_data(write_data));
    initial begin
        ...
        force dut.dot_phase=dot_phase_value;
        for(mode=0;mode<4;mode=mode+1)begin
            range_mode=mode==0;
            dot_mode=mode==2;
            rank_mode=mode==3;
            dot_phase_value=mode==1||mode==2;
                ...
                for(row_value=0;row_value<32;row_value=row_value+1)begin
                    ...
                    for(slot_value=0;slot_value<4;slot_value=slot_value+1)begin
                        ...
                        for(target_lane=0;target_lane<32;target_lane=target_lane+1)begin
                            expected_vector[target_lane*27+:27]=response_data[((target_lane+int'(row_base))&31)*27+:27];
                            relative_lane=(target_lane-int'(column_position)+int'(column_base))&31;
                            if(dot_mode)begin
                                if(relative_lane<8)expected_cache[target_lane*27+:27]=write_data[(relative_lane*4)*27+:27];
                            end
                            else expected_cache[target_lane*27+:27]=write_data[relative_lane*27+:27];
                        ...
                        if(dut.vector_ordered!==expected_vector||dut.cache_ordered!==expected_cache)
                            $fatal(1,"cache rotation mode=%0d shift=%0d row=%0d slot=%0d",mode,shift_value,row_value,slot_value);
                        checks=checks+4;
                    end
                end
            end
