...
module live_operator_memory (
    ...
    wire read_pop=mat_rsp_valid&&mat_rsp_ready;
    // A pending host request cannot steal an active compute command.
    wire ownership_pending=!compute_active&&(p_begin_valid||b_begin_valid||build_req_valid);
    wire read_grant=active&&!ownership_pending&&
        (owner==IDLE||(owner==READ&&mat_dense==read_dense))&&
        (read_count<2||read_pop);
    ...
    always @(posedge clk) begin
        if (!active) begin
            owner <= IDLE; read_count<=0; read_dense <= 1'b0; build_authorized <= 1'b0;
            ...
            case({mat_valid&&mat_ready,read_pop})
            ...
            if (read_pop&&read_count==1&&!(mat_valid&&mat_ready)) owner <= IDLE;
