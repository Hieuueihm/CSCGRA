                        ...
                        for(seq_lane=0;seq_lane<128;seq_lane=seq_lane+1)
                            if(seq_lane+int'(row_start[4:0])>=int'(source_bundle)*32&&
                               ...
                               source_mask[(seq_lane+int'(row_start[4:0]))&31])
                                a_cache[seq_lane]<=vec_rsp_data[((seq_lane+int'(row_start[4:0]))&31)*27+:27];
                        ...
                        if(range_operation) retired_count<=retired_count+1'b1;
                            ...
                            for(lane=0;lane<96;lane=lane+1)
                                if(lane>=int'(col_position-col_start)&&lane<int'(col_position-col_start)+int'(panel_width))
                                    b_cache[lane]<=fabric_out_data[((lane-int'(col_position-col_start))&31)*27+:27];
                        end
                        else if(rank_phase) begin
                            ...
                            if(project) begin
                                for(lane=0;lane<96;lane=lane+1)
                                    if(lane>=int'(col_position-col_start)&&lane<int'(col_position-col_start)+int'(dot_width))
                                        b_cache[lane]<=dot_result_data[((lane-int'(col_position-col_start))&31)*27+:27];
