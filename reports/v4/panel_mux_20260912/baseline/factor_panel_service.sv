    ...
    for(comb_bank=0;comb_bank<32;comb_bank=comb_bank+1)if(request_valid)begin
        if(range_operation)begin
            ...
            if(bank_offset<16&&request_lane<int'(rank_width)*2&&comb_relative<int'(row_count))begin
                panel_req_mask[comb_bank]=1;
                panel_req_addr[comb_bank*9+:9]=9'(3*comb_row+(comb_col>>5));
                if(request_write)panel_req_data[comb_bank*27+:27]=slot_result[request_slot][request_lane*27+:27];
            end
        end
        else begin
                        ...
                        if(!response_is_write) begin
                            for(lane=0;lane<32;lane=lane+1) begin
                                if(range_operation)begin
                                    ...
                                    if(row_value<int'(row_count))factor_response_data[lane*27+:27]=panel_rsp_data[bank*27+:27];
                                    ...
                                    if(lane<int'(dot_width)*4&&row_value<int'(row_end))factor_response_data[lane*27+:27]=panel_rsp_data[bank*27+:27];
                                    else factor_response_data[lane*27+:27]=0;
                                end
                                    ...
                                    bank=(row_value+col_value)&31;
                                    if(lane<int'(rank_width)*2&&row_value<int'(row_end))factor_response_data[lane*27+:27]=panel_rsp_data[bank*27+:27];
                                    else factor_response_data[lane*27+:27]=0;
                                end
                                else begin
                                    ...
                                    bank=(row_value+col_value)&31;
                                    if(lane<int'(current_width))factor_response_data[lane*27+:27]=panel_rsp_data[bank*27+:27];
                                    else factor_response_data[lane*27+:27]=0;
                                end
                            end
