...
# RTL-aware CoSaMP/SP: candidate storage is capped by SPARSE_MAX_K=16.
def run_cosamp(A, y):
    support = []
    x = [0.0] * N
    r = list(y)
    ...
    for _ in range(COSAMP_ITERS):
        ...
        merged = merge_support_hw(support, candidates, K)
...
def main():
    ...
    with (OUT / "noisy_lfsr_24bit_per_algorithm_seed_rtl.csv").open("w", newline="") as f:
        writer = csv.writer(f)
    ...
    with (OUT / "noisy_lfsr_24bit_per_algorithm_seed_rtl.json").open("w") as f:
        json.dump(data, f, indent=2)
    tex = [r"\begin{table}[!t]", r"\centering",
           r"\caption{RTL-aware representative reconstruction quality for $(N,M,K)=(256,64,16)$ using RTL-aware 24-bit fixed-point flow and 20-dB measurement noise. Each algorithm uses its own listed signal seed.}",
           r"\label{tab:reconstruction_quality_rep_rtl}", r"\begin{tabular}{lcccc}", r"\hline",
           r"Algorithm & Seed & MSE & SNR (dB) & Support overlap \\", r"\hline"]
    for r in rows:
        tex.append("{} & {} & ${}$ & {:.2f} & {}/{} \\".format(r["algorithm"], r["seed"], fmt_exp(r["mse"]), r["snr"], r["overlap"], K))
    tex += [r"\hline", r"\end{tabular}", r"\end{table}", ""]
    (OUT / "noisy_lfsr_24bit_per_algorithm_seed_rtl_table.tex").write_text("\n".join(tex))
    for r in rows:
