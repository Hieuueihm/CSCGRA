...
module sb #(
    ...
    output wire [DATA_W-1:0] outPE
);
    switchbox #(.DATA_W(DATA_W), .IDX_W(IDX_W)) u_switchbox (
        .inN(inN), .inS(inS), .inE(inE), .inW(inW), .inPE(inPE),
        .inN_idx({IDX_W{1'b0}}), .inS_idx({IDX_W{1'b0}}), .inE_idx({IDX_W{1'b0}}),
        .inW_idx({IDX_W{1'b0}}), .inPE_idx({IDX_W{1'b0}}), .sel(sel),
        .outN(outN), .outS(outS), .outE(outE), .outW(outW), .outPE(outPE),
        .outN_idx(), .outS_idx(), .outE_idx(), .outW_idx(), .outPE_idx()
    );
endmodule
