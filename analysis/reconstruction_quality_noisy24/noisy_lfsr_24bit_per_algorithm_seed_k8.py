...
def run_gp(A,y):
    ...
    for _ in range(GP_ITERS):
        ...
        atom=pick[0]
        support.append(atom)
        sigma=corr[atom]/max(col_norm[atom],1e-18)
        x[atom]+=sigma
        r=residual(A,y,x)
        bx,bn=best_update(x,r,bx,bn)
    return bx
...
def main():
    ...
    csv_path=OUT/"noisy_lfsr_24bit_per_algorithm_seed_k8.csv"
    ...
    json_path=OUT/"noisy_lfsr_24bit_per_algorithm_seed_k8.json"
