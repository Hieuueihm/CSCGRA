...
MEAS_SNR_DB = 20
MU_SHIFT = 3
ALG_SEEDS = {"OMP": 87, "GOMP": 13, "CoSaMP": 77, "SP": 48, "IHT": 101, "HTP": 20, "GP": 192, "MP": 140}
...
def run_gp(A,y):
    x=[0.0]*N; r=list(y); bx=list(x); bn=norm(r); support=[]
    for _ in range(GP_ITERS):
        ...
        if not support:
            break
        g=[corr[j] for j in support]
        Ad=[sum(A[i][support[t]]*g[t] for t in range(len(support))) for i in range(M)]
        den=dot(Ad,Ad)
        alpha=dot(r,Ad)/den if den>1e-18 else 0.0
...
def main():
    ...
    csv_path=OUT/"noisy_lfsr_24bit_per_algorithm_seed_k8_gp_linesearch_seed192.csv"
    ...
    json_path=OUT/"noisy_lfsr_24bit_per_algorithm_seed_k8_gp_linesearch_seed192.json"
