...
def execute(package,matrix,measurement,limit=100000,*,accelerate_gemv=False,gemv_stats=None,debug_vectors=None):
    ar=Arithmetic(PROFILE);sf=PROFILE.state
    gemv=BoundedGemv(ar,gemv_stats) if accelerate_gemv else None
    matrix=ar.quantize(matrix,PROFILE.coefficient)
    prepared_matrix=gemv.prepare(matrix) if gemv else None
    prepared_b=None
    ...
    def write(v,values,clear_extent=None):
        ...
        # A vector-pool publication owns complete 32-lane words.  Lanes not
        # covered by its mask become invalid, rather than inheriting stale
        # data from a prior wider result in the same word.
    ...
    def kernel(f):
        ...
        if op==23:
            # Feature8 keeps the existing rounded MUL boundary, then applies
            # the loaded ADD/SUB to C.  Check the complete compact image here
            # so this interpreter cannot accept a program the kernel rejects.
            ...
            for lane,context in enumerate(contexts):
                if lane<16:
                    if context!=expected_tail:raise AssertionError('ROUNDED_AFFINE MUL context')
        ...
        if op in (0,16):
            ...
            prepared=prepared_b if q['dense'] else prepared_matrix
            out=(gemv.prepared_matvec(prepared,read(a,length),bool(q['transpose'])) if gemv and prepared is not None else
                 ar.matvec(operator.T if q['transpose'] else operator,read(a,length),PROFILE.coefficient,sf,sf))
        elif op==2:
            ...
            # Feature10 reads a private factor range twice through direct MAC
            # lanes.  It returns the unpatched full tap, the raw factor-square
            # SUM_ACC and an exact tail-nonzero count without a pool source.
            ...
            # raw scalar is returned through dst_s; the public result is the
            # actual full/head tap, never a rounded terminal lane result.
            if (template['descriptor']!=23 or template['bind_a'] or template['bind_b'] or
                    any(context!=297 for context in contexts) or not q['dense'] or
                    q['transpose'] or q['r4'] or q['store_mode'] or q['k_s'] or
            ...
            # The overwritten source lane need not be valid; every retained
            # lane is still read and checked before publication.
            ...
            if (not q['dense'] or q['flags'] or q['aux_length_s'] or length!=m or
                    not 1<=old_support<support<=96 or factors.shape!=(m,old_support) or
                    bmatrix.shape!=(m,support)):
            ...
            factors=np.concatenate((factors,suffix),axis=1)
            count=support
        elif op in (14,15):
            ...
            if op==17:
                ...
                # Validate all source blocks before mutating the private image.
                updates=[]
    ...
    for _ in range(limit):
        ...
        if kind==1:kernel(f)
            ...
            # Reuse programs prove prefix/RHS/job before emitting EXTEND. Keep
            # a private factor prefix only for that opt-in ABI; ordinary BUILD
            # remains destructive and is followed by FACTOR_INIT as before.
            if not package.get('qr_reuse_enabled'):factors=None
            prepared_b=gemv.prepare(bmatrix) if gemv else None
