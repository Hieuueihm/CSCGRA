...
module tb_stream_kernel #(parameter RESIDENT_WRITEBACK=1, parameter GEMV_GROUP_PREFETCH=1,
                          parameter STALLS_ENABLE=1'b1);
    reg  clk;
    reg  rst;
    reg  cancel;
    reg  host_enable;
    ...
    integer wc,rcount,fc,ef,enz,expected_count,cancel_i,resident_remap_cancel,direct_operand_override,factor_range_bad_mask,energy_tap_cancel,project_dot_split_cancel,project_dot_split_fault,bi,slot,block_i,mask_i,i,j,start_cycle,start_candidates,cancel_blocks;
    ...
    always @(negedge clk) begin
        if(STALLS_ENABLE&&request_stalls&&!rst&&busy&&cycles%17<4)force operators.read_grant=1'b0;
        else release operators.read_grant;
        if(STALLS_ENABLE&&!rst&&busy&&cycles%19<3) begin
            force dut.memory.req_ready=1'b0;
            stalls=stalls+1;
        end else release dut.memory.req_ready;
        if(STALLS_ENABLE&&!rst&&busy&&cycles%23<2) force dut.fabric.in_ready=1'b0;
    ...
    initial begin
        ...
        for(c=0;c<ncases;c=c+1) begin
            ...
            factor_range_bad_mask=cancel_i==21;
            // Marker26 is confined to op20 in the standalone PROJECT_DOT A/B
            // wrapper.  It waits until the first eight-column panel has
            // retired and the second DOT panel is presenting valid input.
            project_dot_split_cancel=cancel_i==26;
            project_dot_split_fault=cancel_i==27;
            // Marker25 is confined to op25: wait until private candidate data
            // exists, cancel, and verify that its entire public transaction is
            // invalidated before the following fresh INIT can recover.
            energy_tap_cancel=cancel_i==25;
            if(op!=14&&op!=15&&cancel_i!=6&&cancel_i!=7&&cancel_i!=10&&cancel_i!=16&&cancel_i!=21&&cancel_i!=25&&cancel_i!=26&&cancel_i!=27)begin
                @(negedge clk);rst=1;repeat(2)@(negedge clk);rst=0;
            end
            if(cancel_i==6||cancel_i==7||cancel_i==21)cancel_i=0;
            ...
            req_src_a=0;req_src_b=(op==20||op==21||op==24||op==25||(op==23&&req_scalar_bind_b==32'h0000ffff)) ? 0 : 64;
            ...
            // Marker7 is a test-only vector-Y override for opcode23.
            // support_base returns to canonical zero before DUT acceptance;
            // aux_base remains the functional C source.
            if(op==23&&direct_operand_override) begin
                req_src_b=req_support_base;
                req_support_base=0;
            end
            if(op==1||op==13) begin
                if(dense_i) begin
            ...
            // Marker21 preserves the preceding factor image and corrupts one
            // accepted op24/op25 private-store read mask.  The leaf must reject
            // it before raw terminal/candidate publication.
            if(factor_range_bad_mask)begin
                while(!(dut.panels.state==dut.panels.RUN&&dut.factors.panel_rsp_valid&&dut.factors.panel_rsp_ready)) @(negedge clk);
                force dut.factors.store_rsp_mask=32'd0;
                @(posedge clk);@(negedge clk);release dut.factors.store_rsp_mask;
            end
            // Marker27 injects one fault into the registered output of lane 0
            // of fabric array 0.  Do not force the collapsed 128-bit panel
            // input: force/release on that aggregate can corrupt its resolved
            // port net in XSim.  This leaf register is cleared by abort/reset.
            if(project_dot_split_fault)begin
                while(!(dut.panels.operation==6'd20&&dut.panels.project_phase==0&&
                        dut.panels.col_position==dut.panels.col_start+11'd8&&
                        dut.panels.dot_width<=11'd8&&dut.panels.fabric_out_valid&&
                        dut.panels.fabric_out_ready)) @(negedge clk);
                $display("PROJECT_DOT_SPLIT_FAULT reached tag=%0d col=%0d width=%0d",
                         req_tag,dut.panels.col_position,dut.panels.dot_width);
                force dut.fabric.arrays[0].array_core.pes[0].pe.out_fault=4'd1;
                @(posedge clk);@(negedge clk);
                release dut.fabric.arrays[0].array_core.pes[0].pe.out_fault;
                // With the leaf released, the panel input must immediately
                // reflect fabric's legal upstream aggregate.  Then require
                // abort to clear both views without forcing either bus.
                if(dut.fabric.cascade_run||dut.panels.fabric_out_faults!==dut.fabric.array_out_faults)
                    $fatal(1,"marker27 release did not restore parallel fault aggregate");
                while(dut.state!=dut.FAIL) @(negedge clk);
                if(!dut.fabric.abort)
                    $fatal(1,"marker27 kernel FAIL did not assert fabric abort");
                @(posedge clk);@(negedge clk);
                if(dut.fabric.arrays[0].array_core.pes[0].pe.out_fault!==4'd0||
                   dut.panels.fabric_out_faults!==dut.fabric.array_out_faults||dut.fabric.array_out_faults!==128'd0)
                    $fatal(1,"marker27 abort did not clear released leaf fault");
                cancel_i=0;
            end
            ...
            if(cancel_i!=0) begin
                ...
                else if(energy_tap_cancel) begin
                    cancel_blocks=(len_i+31)>>5;
                    while(!(dut.state==dut.SUPPORT_WAIT&&dut.panels.state==dut.panels.CANDIDATE&&
                            dut.scratch_masks[0]!=0)) @(negedge clk);
                end
                ...
                else if(project_dot_split_cancel) begin
                    while(!(dut.panels.operation==6'd20&&dut.panels.project_phase==0&&
                            dut.panels.col_position==dut.panels.col_start+11'd8&&
                            dut.panels.dot_width<=11'd8&&dut.panels.fabric_in_valid)) @(negedge clk);
                    $display("PROJECT_DOT_SPLIT_CANCEL reached tag=%0d col=%0d width=%0d",
                             req_tag,dut.panels.col_position,dut.panels.dot_width);
                end
                else if(energy_tap_cancel) begin end
                ...
                if(cancel_i==19||cancel_i==20||energy_tap_cancel)
                ...
                if(rsp_fault!==ef||rsp_job!==7||rsp_tag!==c+1||rsp_fmt!==1||
                    ...
                    $fatal(1,"response case%0d op%0d fault%0d/%0d detail%0d scalar%h/%h nz%0d/%0d",c,op,rsp_fault,ef,rsp_detail,rsp_data,expected_scalar,rsp_nonzero,enz);
