...
module tb_support_service;
    ...
    wire vec_valid,vec_ready,vec_rsp_ready,candidate_valid,candidate_ready;
    wire [8:0] vec_block;
    ...
    integer cycles=0,reads=0,writes=0,stalls=0,checks=0;
    integer fd,trace_fd,rc,blocks,i,j,number,stall_mode,inject,cancel_mode;
    ...
    wire grant = !stall_mode || cycles%7!=0;
    wire response_grant = !hold_response && (!stall_mode || cycles%9!=0);
    wire write_grant = !hold_write && (!stall_mode || cycles%5!=0);
    ...
    support_service dut(
        ...
        .req_length(req_length),.req_k(req_k),.req_scalar_a(req_scalar_a),.req_support_base(req_support_base),.req_aux_base(req_aux_base),
    ...
    always @(posedge clk) begin
        cycles<=cycles+1;
        if(cycles>2000000) $fatal(1,"watchdog");
        if(rst || cancel) begin
            previous_read<=0;previous_write<=0;previous_done<=0;
        end else begin
            if(previous_read && {vec_block,vec_mask,vec_tag}!==held_read) $fatal(1,"read request changed while held");
            if(previous_write && {candidate_block,candidate_mask,candidate_data}!==held_write) $fatal(1,"candidate changed while held");
            if(previous_done && {rsp_fault,rsp_detail,rsp_count,rsp_length,rsp_data,rsp_nonzero,rsp_job,rsp_tag,rsp_fmt}!==held_done) $fatal(1,"response changed while held");
    ...
    task launch;
        begin
            ...
            req_support_length=saved_s;req_aux_length=saved_t;req_index=saved_index;req_fmt=saved_fmt;req_scalar_a=saved_scalar;
            req_job=16'h1234;req_tag=16'h5678;
            req_valid=1;
    ...
    initial begin
        ...
        if(!fd || !trace_fd) $fatal(1,"files");
        rc=$fscanf(fd,"%d %d %d %d %d %d %d %d %d %d %d %d\n",saved_op,saved_length,saved_k,saved_flags,saved_s,saved_t,saved_index,saved_fmt,stall_mode,inject,cancel_mode,saved_scalar);
        if(rc!=12) $fatal(1,"fixture header");
        rc=$fscanf(fd,"%d %d %d\n",source_address,support_address,auxiliary_address);
        if(rc!=3) $fatal(1,"fixture bases");
        req_src_a=source_address;req_support_base=support_address;req_aux_base=auxiliary_address;
        rc=$fscanf(fd,"%d\n",blocks);
        for(i=0;i<480;i=i+1) valid_masks[i]=0;
        repeat(3) tick();rst=0;tick();
        for(i=0;i<blocks;i=i+1) begin
            rc=$fscanf(fd,"%d %h %h\n",number,host_mask,host_data);
            if(rc!=3) $fatal(1,"fixture memory");
            host_block=number;host_valid=1;
            while(!memory_ready) tick();
            tick();host_valid=0;valid_masks[number]=host_mask;
        end
        active=1;
        ...
        if(cancel_mode!=0) begin
            ...
            endcase
            repeat(3) tick();
            if(cancel_mode==5) rst=1;else cancel=1;
            tick();
            if(rsp_valid || vec_valid || candidate_valid || memory_valid) $fatal(1,"flush");
            rst=0;cancel=0;hold_read=0;hold_response=0;hold_write=0;
            repeat(3) tick();
            $fdisplay(trace_fd,"C");
            launch();
        end
        ...
        repeat(7) tick();
        if(rsp_job!==16'h1234 || rsp_tag!==16'h5678 || rsp_fmt!==saved_fmt[7:0]) $fatal(1,"identity");
        ...
        repeat(3) tick();
        if(rsp_valid || memory_valid || !req_ready) $fatal(1,"retirement");
        $fdisplay(trace_fd,"END %0d %0d %0d %0d %0d",cycles,reads,writes,stalls,checks);
        $display("PASS cycles=%0d reads=%0d writes=%0d",cycles,reads,writes);
