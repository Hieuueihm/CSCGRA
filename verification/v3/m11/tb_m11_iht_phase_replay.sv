...
module tb_m11_iht_phase_replay;
    reg clk = 1'b0;
    always #5 clk = ~clk;
    reg rst_n = 1'b0;
    integer failures = 0;
    ...
    reg [7:0] expected_entry [0:13];

    reg start_request = 1'b0;
    reg [63:0] start_address = 64'h2000;
    ...
    reconstruction_phase_controller dut (
        ...
        .iteration_limit_reached(launch_count >= 14), .support_stable(1'b0),
        ...
        .outer_iter(16'd2), .solver_iter(8'd0), .engine_busy(engine_busy),
        .phase_active(phase_active), .csr_cfg_fetch_active(csr_cfg_fetch_active),
    ...
    initial begin
        $readmemh("program_02_phase.mem", phase_memory);
        expected_entry[0] = 8'd85;
        expected_entry[1] = 8'd92;
        expected_entry[2] = 8'd96;
        expected_entry[3] = 8'd102;
        expected_entry[4] = 8'd112;
        expected_entry[5] = 8'd116;
        expected_entry[6] = 8'd10;
        expected_entry[7] = 8'd85;
        expected_entry[8] = 8'd92;
        expected_entry[9] = 8'd96;
        expected_entry[10] = 8'd102;
        expected_entry[11] = 8'd112;
        expected_entry[12] = 8'd116;
        expected_entry[13] = 8'd10;
        ...
        check(cfg_start_valid, "IHT phase requests configuration");
        @(negedge clk); cfg_start_ready = 1'b1;
        @(posedge clk);
        @(negedge clk); cfg_start_ready = 1'b0; active_cfg_valid = 1'b1;

        timeout = 0;
        while (!done_pulse && !error_pulse && timeout < 600) begin
            @(posedge clk);
            timeout = timeout + 1;
        end
        check(!error_pulse, "IHT phase replay has no controller error");
        check(done_pulse, "IHT phase replay terminates");
        check(stop_reason == `RECON_STOP_ITERATION_LIMIT,
              "IHT phase stops on iteration limit");
        check(launch_count == 14, "IHT phase completes exactly two iterations");
        check(trace_count >= 34, "IHT phase emits instruction trace");
        check(active_cfg_release, "IHT phase releases active configuration");
        if (failures == 0)
            $display("M11 IHT PHASE IMAGE REPLAY PASS launches=%0d traces=%0d",
                     launch_count, trace_count);
        else
            $display("FAIL: M11 IHT phase replay failures=%0d", failures);
        $finish;
    end
endmodule
