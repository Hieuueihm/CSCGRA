...
module tb_m11_gp_phase_replay;
    ...
    always @(posedge clk) begin
        ...
        if (array_launch_valid) begin
            check(launch_count < 22, "GP phase emitted no extra array launch");
            if (launch_count < 22)
                check(array_entry_pc == expected_entry[launch_count],
                      "GP phase launch order matches resident context ABI");
    ...
    reconstruction_phase_controller dut (
        ...
        .start_address(64'h5000), .abort_request(1'b0),
        ...
        .exec_error_detail(32'd0), .active_support(7'd2),
    ...
    initial begin
        $readmemh("program_05_phase.mem", phase_memory);
        append_iteration(0);
        append_iteration(11);
        ...
        check(cfg_start_valid, "GP phase requests configuration");
        ...
        check(!error_pulse, "GP phase replay has no controller error");
        check(done_pulse, "GP phase replay terminates");
        check(stop_reason == `RECON_STOP_ITERATION_LIMIT,
              "GP phase stops on iteration limit");
        check(launch_count == 22, "GP phase completes exactly two iterations");
        check(trace_count >= 48, "GP phase emits instruction trace");
        check(active_cfg_release, "GP phase releases active configuration");
        if (failures == 0)
            $display("M11 GP PHASE IMAGE REPLAY PASS launches=%0d traces=%0d",
                     launch_count, trace_count);
        else
            $display("FAIL: M11 GP phase replay failures=%0d", failures);
        $finish;
    end
endmodule
