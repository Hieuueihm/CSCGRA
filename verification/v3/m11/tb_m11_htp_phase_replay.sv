...
module tb_m11_htp_phase_replay;
`ifdef GOMP_PHASE
    localparam integer PHASE_WORDS = 37;
    localparam integer EXPECTED_LAUNCHES = 34;
`else
    localparam integer PHASE_WORDS = 39;
    localparam integer EXPECTED_LAUNCHES = 36;
`endif
    reg clk = 1'b0;
    always #5 clk = ~clk;
    reg rst_n = 1'b0;
    ...
    reconstruction_phase_controller dut (
        ...
        .iteration_limit_reached(launch_count >= EXPECTED_LAUNCHES),
        .support_stable(1'b0),
    ...
    initial begin
`ifdef GOMP_PHASE
        $readmemh("program_06_phase.mem", phase_memory);
`else
        $readmemh("program_03_phase.mem", phase_memory);
`endif
        for (outer_index = 0; outer_index < 2; outer_index = outer_index + 1) begin
`ifdef GOMP_PHASE
            launch_index = outer_index * 17;
            expected_entry[launch_index+0] = 8'd85;
            expected_entry[launch_index+1] = 8'd208;
            expected_entry[launch_index+2] = 8'd214;
            expected_entry[launch_index+3] = 8'd116;
            expected_entry[launch_index+4] = 8'd37;
            expected_entry[launch_index+5] = 8'd16;
            expected_entry[launch_index+6] = 8'd44;
            expected_entry[launch_index+7] = 8'd30;
            expected_entry[launch_index+8] = 8'd59;
            expected_entry[launch_index+9] = 8'd68;
            expected_entry[launch_index+10] = 8'd16;
            expected_entry[launch_index+11] = 8'd44;
            expected_entry[launch_index+12] = 8'd30;
            expected_entry[launch_index+13] = 8'd59;
            expected_entry[launch_index+14] = 8'd120;
            expected_entry[launch_index+15] = 8'd124;
            expected_entry[launch_index+16] = 8'd10;
`else
            launch_index = outer_index * 18;
            expected_entry[launch_index+0] = 8'd85;
            expected_entry[launch_index+1] = 8'd92;
            expected_entry[launch_index+2] = 8'd96;
            expected_entry[launch_index+3] = 8'd102;
            expected_entry[launch_index+4] = 8'd116;
            expected_entry[launch_index+5] = 8'd37;
            expected_entry[launch_index+6] = 8'd16;
            expected_entry[launch_index+7] = 8'd44;
            expected_entry[launch_index+8] = 8'd30;
            expected_entry[launch_index+9] = 8'd59;
            expected_entry[launch_index+10] = 8'd68;
            expected_entry[launch_index+11] = 8'd16;
            expected_entry[launch_index+12] = 8'd44;
            expected_entry[launch_index+13] = 8'd30;
            expected_entry[launch_index+14] = 8'd59;
            expected_entry[launch_index+15] = 8'd120;
...
`endif
        ...
        check(cfg_start_valid, "HTP phase requests configuration");
        ...
        check(!error_pulse, "HTP phase replay has no controller error");
        check(done_pulse, "HTP phase replay terminates");
        check(stop_reason == `RECON_STOP_ITERATION_LIMIT,
              "HTP phase stops on iteration limit");
        check(launch_count == EXPECTED_LAUNCHES,
              "phase completes two outer iterations");
        check(certificate_count == 4, "HTP phase completes two refinement iterations per outer loop");
        check(trace_count >= 70, "HTP phase emits instruction trace");
        check(active_cfg_release, "HTP phase releases active configuration");
        if (failures == 0) begin
`ifdef GOMP_PHASE
            $display("M11 GOMP PHASE IMAGE REPLAY PASS launches=%0d traces=%0d certificates=%0d",
                     launch_count, trace_count, certificate_count);
`else
            $display("M11 HTP PHASE IMAGE REPLAY PASS launches=%0d traces=%0d certificates=%0d",
                     launch_count, trace_count, certificate_count);
`endif
        end else
            $display("FAIL: M11 HTP phase replay failures=%0d", failures);
        $finish;
    end
endmodule
