    ...
    generate
        for (r = 0; r < ROWS; r = r + 1) begin : gen_r
            for (lc = 0; lc < CLUSTER_COLS; lc = lc + 1) begin : gen_c
                ...
                // Row 0 is the only controller ingress. Lower-row datapath
                // selects are driven by the registered token valid, never by
                // the controller opcode. This removes the context-memory
                // control path into the lower-row DSP inputs.
                wire phys_corr_pe_active = (r == 0) ? corr_mode_active : corr_token_active;
                wire corr_clear_active = corr_mode_active && corr_acc_clear;
                wire resid_token_active = (r == 0) ?
                ...
                wire tile_acc_clear = ls_wide_active_row ? 1'b0 : (corr_clear_active ? 1'b1 :
                                          (phys_corr_pe_active ? 1'b0 :
                ...
                wire tile_en = ls_wide_active_row || corr_clear_active ||
                               (ctx_valid && row_en[r] && lane_valid[lc]) ||
                               phys_ls_pe_active || mesh_ctx_active || (|mesh_keepalive);
                ...
                pe_tile #(
                    ...
                    .corr_pipeline_active(corr_mode_active),
