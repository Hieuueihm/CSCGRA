module sparse_kernel_service_engine #(
    ...
    // Pair-mode correlation extensions passed through to the PE array.
    output wire [COLS*DATA_W-1:0]   ls_pe_rhs_phi2_bus,
    output wire                     ls_corr_pair_mode,
    output wire                     ls_corr_half_sel,
    ...
    output wire [COLS*DATA_W-1:0]    ls_wide_a_row0_bus,
    output wire [COLS*DATA_W-1:0]    ls_wide_a_row1_bus,
    output wire [COLS*DATA_W-1:0]    ls_wide_a_row2_bus,
    output wire [COLS*DATA_W-1:0]    ls_wide_a_row3_bus,
    output wire [COLS*DATA_W-1:0]    ls_wide_b_row0_bus,
    output wire [COLS*DATA_W-1:0]    ls_wide_b_row1_bus,
    output wire [COLS*DATA_W-1:0]    ls_wide_b_row2_bus,
    output wire [COLS*DATA_W-1:0]    ls_wide_b_row3_bus,
    ...
    sparse_loop_controller #(.COLS(COLS), .DATA_W(DATA_W), .SCALAR_W(SCALAR_W), .MEM_AW(MEM_AW), .IDX_W(IDX_W), .MAX_M(MAX_M), .MAX_N(MAX_N), .MAX_K(MAX_K), .REFINE_ITERS(REFINE_ITERS)) u_sparse_loop_controller (
        ...
        .pe_rhs_product_bus(pe_rhs_product_bus), .pe_corr_acc_bus(pe_corr_acc_bus), .last_result_value(last_result_value), .last_result_idx(last_result_idx), .pe_rhs_phi_bus(ls_pe_rhs_phi_bus), .pe_rhs_y_bus(ls_pe_rhs_y_bus), .pe_rhs_phi2_bus(ls_pe_rhs_phi2_bus), .corr_pair_mode(ls_corr_pair_mode), .corr_half_sel(ls_corr_half_sel), .pe_rhs_active(ls_pe_rhs_active), .pe_sparse_clear(pe_sparse_clear), .pe_corr_acc_clear(pe_corr_acc_clear), .pe_corr_acc_en(pe_corr_acc_en), .pe_sparse_op(pe_sparse_op),
        ...
        .ls_wide_product_bus(ls_wide_product_bus),
