...
def run_reference_alg(tb_alg: int, phi: list[list[int]], y: list[int], n_size: int, k_size: int, scale_q: int) -> list[int]:
    ...
    for _ in range(k_size):
        if tb_alg == 0:  # OMP
            ...
            x, residual = ls_fit_quantized(phi, y, support, n_size)
        elif tb_alg == 2:  # IHT, RTL uses MU_SHIFT=2
            scores = corr_scores(phi, residual, n_size)
            ...
            scores = corr_scores(phi, residual, n_size)
            x = [sat_s24(x[col_idx] + s24(scores[col_idx])) for col_idx in range(n_size)]
            support = prune_by_abs(x, k_size)
            x, residual = ls_fit_quantized(phi, y, support, n_size)
...
def main() -> None:
    ...
    lines: list[str] = []
    lines.append("// Auto-generated by scripts/export_k_sweep_golden_reference.py")
    lines.append("// Case 0 is cross-checked against immutable golden_model/golden_cases.vh final K=16 snapshot.")
    ...
    for case_idx, (m_size, n_size, k_size) in enumerate(CASES):
        if (m_size, n_size, k_size) == (64, 256, 16):
            iter_idx = 15
            for tb_alg, immutable_alg in enumerate(TB_TO_IMMUTABLE_ALG):
                for elem_idx in range(n_size):
                    immutable_key = ((immutable_alg * max_iters + iter_idx) * max_n) + elem_idx
                    value = immutable_x.get(immutable_key, 0)
                    if value:
