    ...
    generate
        for (r = 0; r < ROWS; r = r + 1) begin : gen_r
            for (lc = 0; lc < CLUSTER_COLS; lc = lc + 1) begin : gen_c
                ...
                wire phys_ls_pe_active = !ls_wide_mul_active && (phys_corr_pe_active || phys_refine_pe_active);
                ...
                wire corr_row_owner = phys_corr_pe_active && (corr_slot == ROW_SLOT);
                ...
                wire [3:0] tile_pe_op = ls_wide_mul_active ? 4'd0 : (mesh_ctx_active ? mesh_ctx_row_op :
                                          (phys_corr_pe_active ? 4'd0 :
                                          (phys_refine_pe_active ? (row0_mac_stage ? 4'd0 : (row1_accum_stage ? 4'd1 : 4'd8)) : pe_op)));
                wire [2:0] tile_src_a_sel = ls_wide_mul_active ? 3'd7 : (phys_corr_pe_active ? 3'd7 :
                                             (phys_refine_pe_active ? (row0_mac_stage ? 3'd7 : 3'd0) : src_a_sel));
                wire [2:0] tile_src_b_sel = ls_wide_mul_active ? 3'd7 : (phys_corr_pe_active ? 3'd7 :
                ...
                wire tile_acc_clear = ls_wide_mul_active ? 1'b0 : (phys_corr_pe_active ? corr_acc_clear :
                                          (phys_refine_pe_active ? (r == 0) : (ctx_valid && row_en[r] && acc_clear && first_in_phase)));
                wire tile_acc_en = ls_wide_mul_active ? 1'b0 : (phys_corr_pe_active ? (corr_acc_en && corr_row_owner) :
                ...
                wire tile_ext_b_is_scalar = ls_wide_mul_active ? 1'b1 : (phys_corr_pe_active ? 1'b1 :
                                                (phys_refine_pe_active ? (r == 0) : ext_ctrl[0]));
                wire tile_en = ls_wide_mul_active || (ctx_valid && row_en[r] && lane_valid[lc]) || phys_ls_pe_active || (mesh_ctx_active && row_en[r]) || (|mesh_keepalive);
                wire [DATA_W-1:0] tile_phi_data = ls_wide_mul_active ?
                    ls_wide_a_bus[CELL*DATA_W +: DATA_W] : phi_bus[lc*DATA_W +: DATA_W];
                wire [DATA_W-1:0] tile_scalar_data = ls_wide_mul_active ?
                    ls_wide_b_bus[CELL*DATA_W +: DATA_W] : scalar_bus[lc*DATA_W +: DATA_W];
