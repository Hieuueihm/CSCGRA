...
DEFAULT_OUT = ROOT / "verification" / "v2" / "run1" / "k_sweep_golden_mu3.vh"
...
def generate(output_path: Path) -> str:
    ...
    lines: list[str] = []
    lines.append("// Auto-generated by models/golden/generate_k_sweep_golden.py")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate the canonical v2 K-sweep golden include.")
    parser.add_argument("--output", type=Path, default=DEFAULT_OUT,
                        help="output include path (default: verification/v2/run1/k_sweep_golden_mu3.vh)")
    parser.add_argument("--check", action="store_true",
                        help="compare generated contents with the existing output without changing it")
    args = parser.parse_args()
    output_path = args.output if args.output.is_absolute() else ROOT / args.output
    generated = generate(output_path) if not args.check else generate(output_path.with_suffix(output_path.suffix + ".tmp"))
    if args.check:
        existing = output_path.read_text() if output_path.exists() else None
        output_path.with_suffix(output_path.suffix + ".tmp").unlink()
        if existing != generated:
            raise SystemExit(f"golden mismatch: {output_path}")
        print(f"golden check PASS: {output_path}")
    else:
        print(output_path)
