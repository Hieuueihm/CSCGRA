...
textbook/canonical sign-off.  The active regression continues to use
`verification/v2/run1/k_sweep_golden_mu3.vh` for historical hardware sign-off.
Canonical GP is now available as an isolated opt-in RTL mode; CoSaMP/SP K16
remain intentionally deferred because that capacity is not required.
...
The canonical GP sweep covers all eight configured cases and reports 48/48
checks PASS with zero `X_MISM` records.  Cases 0--7 complete in 75,297;
...
Until the timing/resource gate is approved, the default sign-off still calls
GP the **RTL-compatible hardware variant**.  The isolated canonical mode is
available for audit; CoSaMP/SP K16 remain hardware variants and are not part
of this migration.
